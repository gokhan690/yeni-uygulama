...
const SAVE_KEY = "hayatYoluSave_v1";
...
function checkDeath() {
  let cause = null;
  if (S.stats.health <= 0) cause = pick(DATA.causesOfDeath.health);
  else if (S.age >= 60) {
...
function openActivities() {
  if (guardPrison()) return;
  let b = `<div class="section-title">Kendine Zaman Ayır</div>`;
...
function save() {
  if (!S || !S.alive) return;
  try { localStorage.setItem(SAVE_KEY, JSON.stringify(S)); } catch (e) { /* depolama yok */ }
}
...
function load() {
  try {
    ...
    // cond fonksiyonları DATA'da kaldığı için state temiz; trait'i tazele
...
function init() {
  ...
  if (saved) {
    ...
    $("btn-continue").onclick = () => startGame(saved);
  }
